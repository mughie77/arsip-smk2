...
<div class="mb-8">
    <h1 class="text-3xl font-bold text-slate-900 tracking-tight">Daftar Notulen</h1>
    <p class="text-slate-500 mt-1 font-medium">Dokumentasi hasil rapat dan kegiatan institusi secara terpusat.</p>
</div>

<!-- Area Aksi dan Filter -->
<div class="flex flex-col xl:flex-row gap-6 mb-8">
    <div class="flex flex-wrap gap-2">
        <button type="button" class="inline-flex items-center gap-1.5 bg-accent hover:bg-accent/90 text-white px-3.5 py-2 rounded-lg text-sm font-bold shadow-md shadow-accent/20 transition-all active:scale-95" data-bs-toggle="modal" data-bs-target="#notulenModal" id="btnTambah">
            <i class="fas fa-plus text-xs"></i> Tambah Data
        </button>
        <a href="../core/export_xlsx.php?jenis=notulen&dari=<?php echo $dari_tanggal; ?>&sampai=<?php echo $sampai_tanggal; ?>&keyword=<?php echo urlencode($keyword); ?>" class="inline-flex items-center gap-1.5 bg-emerald-500 hover:bg-emerald-600 text-white px-3.5 py-2 rounded-lg text-sm font-bold shadow-md shadow-emerald-500/20 transition-all active:scale-95">
            <i class="fas fa-file-excel text-xs"></i> Ekspor XLSX
        </a>
    </div>

    <div class="flex-1">
        <form method="GET" action="notulen.php" class="bg-white p-2 rounded-2xl shadow-sm border border-slate-100 flex flex-wrap xl:flex-nowrap gap-2">
            <div class="relative flex-1 min-w-[200px]">
                <i class="fas fa-search absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"></i>
                <input type="text" class="w-full bg-slate-50 border-none focus:ring-2 focus:ring-accent rounded-xl pl-11 pr-4 py-2 text-sm font-medium text-slate-700" name="keyword" placeholder="Cari nama kegiatan..." value="<?php echo htmlspecialchars($keyword); ?>">
            </div>
            ...
            <a href="notulen.php" class="bg-slate-100 text-slate-600 px-4 py-2 rounded-xl font-bold hover:bg-slate-200 transition-all flex items-center justify-center"><i class="fas fa-sync-alt"></i></a>
        </form>
    </div>
</div>
...
<div class="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden">
    <div class="px-8 py-6 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
        <h3 class="font-bold text-slate-900 flex items-center gap-3">
            ...
            Daftar Notulen Rapat/Kegiatan
        </h3>
        <div class="flex items-center gap-4">
            <form method="GET" action="notulen.php" class="flex items-center gap-3">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/50 text-slate-400 text-[11px] uppercase tracking-[0.2em] font-black">
                        ...
                        <th class="px-6 py-5 border-b border-slate-100"><?php echo sortable_header('Tanggal', 'tanggal', $sort_by, $sort_dir); ?></th>
                        <th class="px-6 py-5 border-b border-slate-100"><?php echo sortable_header('Kegiatan', 'kegiatan', $sort_by, $sort_dir); ?></th>
                        ...
                        <?php while ($row = mysqli_fetch_assoc($result)) : ?>
                            <tr class="hover:bg-slate-50/80 transition-colors group text-sm md:text-base">
                                ...
                                <td data-label="Tanggal" class="px-6 py-5">
                                    <span class="inline-flex bg-amber-50 text-amber-600 px-3 py-1 rounded-full text-xs font-bold tracking-tight border border-amber-100">
                                        <?php echo date('d M Y', strtotime($row['tanggal'])); ?>
                                    </span>
                                </td>
                                <td data-label="Kegiatan" class="px-6 py-5 text-sm font-bold text-slate-900"><?php echo htmlspecialchars($row['kegiatan']); ?></td>
                                <td data-label="Berkas" class="px-6 py-5 text-center md:text-left">
                                    <?php if (!empty($row['nama_file'])) : ?>
                                        <a href="../uploads/notulen/<?php echo htmlspecialchars($row['nama_file']); ?>" target="_blank" class="inline-flex items-center gap-2 text-accent hover:text-accent/80 font-bold text-xs transition-colors">
                                ...
                                <td data-label="Aksi" class="px-8 py-5 text-right">
                                    <div class="flex items-center justify-end gap-2 transition-opacity">
                                        <a href="notulen_edit.php?id=<?php echo $row['id']; ?>" class="w-10 h-10 flex items-center justify-center rounded-xl bg-amber-50 text-amber-500 hover:bg-amber-500 hover:text-white transition-all shadow-md">
                                            <i class="fas fa-edit text-base"></i>
                                        </a>
                                        <form action="../core/notulen_aksi.php" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?');">
    ...
    <div class="px-8 py-6 bg-slate-50/30 border-t border-slate-50 flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        <nav class="flex items-center gap-2">
            ...
            $prev_link = $prev_disabled ? '#' : 'notulen.php?' . http_build_query($prev_params);
            ?>
            ...
            for ($i = 1; $i <= $total_pages; $i++) {
                if ($i == 1 || $i == $total_pages || ($i >= $page - $range && $i <= $page + $range)) {
                    ...
                    echo "<a href='notulen.php?" . http_build_query($page_params) . "' class='w-10 h-10 flex items-center justify-center rounded-xl border font-bold text-sm transition-all $activeClass'>$i</a>";
            ...
            $next_link = $next_disabled ? '#' : 'notulen.php?' . http_build_query($next_params);
            ?>

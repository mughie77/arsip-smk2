...
function redirect_with_message($message, $status) {
    header("Location: ../admin/surat_keluar.php?message=" . urlencode($message) . "&status=" . $status);
    exit;
}
...
if ($action == 'delete') {
    $id = (int)($_GET['id'] ?? 0);
    if ($id === 0) redirect_with_message("ID tidak valid.", "error");
    ...
    if (mysqli_stmt_execute($stmt)) {
        redirect_with_message("Surat keluar berhasil dihapus.", "success");
    } else {
        redirect_with_message("Error saat menghapus: " . mysqli_stmt_error($stmt), "error");
    }
    mysqli_stmt_close($stmt);
    exit;
}
...
if ($action == 'add' || $action == 'edit') {
    ...
    if (empty($nomor_surat) || empty($klasifikasi_id) || empty($tujuan_surat) || empty($perihal) || empty($tanggal_kirim)) {
        redirect_with_message("Semua field wajib diisi.", "error");
    }
    ...
    if (empty($kode_klasifikasi)) redirect_with_message("Klasifikasi tidak valid.", "error");
    ...
    if (empty($kode_sekolah)) redirect_with_message("Pengaturan sekolah tidak ditemukan.", "error");
    ...
    if (isset($_FILES['nama_file_pdf']) && $_FILES['nama_file_pdf']['error'] == UPLOAD_ERR_OK) {
        ...
        if ($_FILES['nama_file_pdf']['size'] > 3 * 1024 * 1024) redirect_with_message("Ukuran file maksimal 3 MB.", "error");
        if (strtolower(pathinfo($_FILES['nama_file_pdf']['name'], PATHINFO_EXTENSION)) != 'pdf') redirect_with_message("Hanya file PDF yang diizinkan.", "error");
        ...
        if (move_uploaded_file($_FILES['nama_file_pdf']['tmp_name'], $upload_dir . $new_file_name)) {
            ...
            redirect_with_message("Gagal mengunggah file baru.", "error");
        }
    }

    if ($action == 'add') {
        if (empty($nama_file_pdf_final)) redirect_with_message("File PDF wajib diunggah.", "error");
    ...
    if (mysqli_stmt_execute($stmt)) {
        redirect_with_message($success_msg, "success");
    } else {
        redirect_with_message("Error: " . mysqli_stmt_error($stmt), "error");
    }
    mysqli_stmt_close($stmt);
